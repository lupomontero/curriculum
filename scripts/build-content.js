...
const readMeta = async (dir) => {
  const es = fs.existsSync(path.join(dir, 'README.md'));
  const pt = fs.existsSync(path.join(dir, 'README.pt-BR.md'));
...
const readDir = async (dir) => {
  ...
  const filtered = files.filter(f => !/^[\._]/.test(f));
  const basename = path.basename(dir);
  const type = basename.slice(0, -1);

  const foo = await Promise.all(filtered.map(f => {
    return readMeta(path.join(dir, f));
  }));
...
const main = async () => {
  const filterDirs = files => files.filter(f => !/^[\._]/.test(f));

  const dirs = await Promise.all([
    readDir(path.resolve(__dirname, '../projects')),
    readDir(path.resolve(__dirname, '../topics')),
  ])
  // .then(([projects, topics]) => {

  // })
  // .then(dirs => dirs.reduce((memo, dirs) => memo.concat(dirs), []));
